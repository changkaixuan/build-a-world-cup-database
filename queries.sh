...
echo "$($PSQL "(SELECT name FROM teams LEFT JOIN games ON teams.team_id = games.winner_id WHERE year=2014) UNION (SELECT name FROM teams LEFT JOIN games ON teams.team_id = games.opponent_id WHERE year=2014) ORDER BY name ")"
